...
\begin{figure}[!t]
    \centering
        \includegraphics[width=0.7\linewidth]{virtuous_circle.png}
...
\begin{figure}[!ht]
\centering
    \subfloat[][Section of Poppy's leg]{\label{fig:Poppy_leg_section}\includegraphics[height=3cm]{Poppy_leg_section.jpg}}
    \hfil
    \subfloat[][Equivalent rectangular section]{\label{fig:basic_leg_section}\includegraphics[height=3cm]{basic_leg_section.jpg}}
...
\begin{figure}[!tb]
\centering
    \subfloat[][Poppy's arm mechanical structure]{\label{fig:poppy_arm_truss}\includegraphics[width=0.6\linewidth]{arm_truss.jpg}}


    \subfloat[][Poppy's leg mechanicalt structure]{\label{fig:poppy_leg_truss}\includegraphics[width=0.6\linewidth]{leg_truss.jpg}}
...
The design and the choice we made have raised several limitations:

\subsection{Simplicity} % (fold)
Creating a robot those one of the main objective is to be easy to duplicate implies strong constraints on the design. As we saw in chapter~\ref{cha:experimental-methods}, humanoid robots have often a complex design involving many components. Achieving such complexity is not possible if we want to keep the robot easy and quick to assemble by non-expert users. Therefore, Poppy's mechanics is simple, with only one part per motor. However they are several joints those performance could be improved by changing the design. For example, just by adding a complementary mechanism such as a reduction gear, we could increase the applied torque on critical joint (e.g. knee, ankle, hip).

\subsection{No hands} % (fold)
The current version of Poppy does not have articulated hands. The grasping ability was not a top priority and is challenging, so we preferred to only design fixed hand. They are several laboratories working on this topic so we hope one of them will be interested to contribute in the Poppy project and design articulated hands. We already have had contact with the Bristol Robotics Lab and the Open hand project for this purpose.

\subsection{Motor modularity} % (fold)
The parametric modeling is great if we are we want to create variation of the same pattern with different dimensions. Thus, we can easily create configuration of all Robotis Dynamixel motors. Yet, if we are interested to use another motor brand, the configuration will not be direct and we will have to redo a part of the design process to ensure the compatibility.


\subsection{Electronic} % (fold)
\label{sub:electronic-limitations}

The electronics we developed is compatible with the exploration of morphology for sensory space because they have a lot of I/O ports to extend Poppy's sensorimotor space. Yet it has several limitations that make the final solution unsatisfactory.
...
Thanks to the methodology presented in the chapter~\ref{cha:methodology}, the design and production of a completely new humanoid robot has been very fast and low cost. Indeed, the project began at the end of May 2012 and the first fully-functional version of Poppy (the one we can see in \figurename~\ref{fig:TER_cognitic}) has been presented at the end of September 2012 at Collège de France. By cumulating the work of the different people involved, it is equivalent to about 8 months and cost less than \texteuro10,000 in material.

Because of the lack of easy and cheap tooling to produce custom electronic board, the new design of the electronic architecture took more time and is still in progress for few elements.

TODO Add photo of the kit

Yet Poppy cost about \texteuro8,000 and it only takes one or two day to be assembled (see REF). Also its morphology design allows for modularity both for the mechanics as the parametric parts can be easily customized and reprint, and for the electronic thanks to the compatibility with the Arduino environment. This modularity is completed by the control library those will be presented in the next chapter.

Example of Poppy morphology's variations will be presented in chapter~\ref{cha:changing_morphology}. Also thanks to its simple design, Poppy can be relevant for educational and artistic purposes. Applications will be presented in chapter~\ref{cha:education} and chapter~\ref{cha:art}.
